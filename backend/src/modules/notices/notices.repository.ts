...
export class NoticesRepository {
  ...
  static async update(params: { societyId: string; noticeId: string; patch: any }) {
    ...
    const sets: string[] = [];
    const vals: any[] = [params.noticeId, params.societyId];
    let idx = 3;
    ...
    if (!sets.length) {
      const r0 = await pool.query(`SELECT * FROM notices WHERE id=$1 AND society_id=$2`, [params.noticeId, params.societyId]);
  ...
  static async delete(params: { societyId: string; noticeId: string }) {
    const r = await pool.query(`DELETE FROM notices WHERE id=$1 AND society_id=$2`, [params.noticeId, params.societyId]);
  ...
  static async list(params: { societyId: string; limit: number; offset: number }) {
    const q = `
      ...
      LIMIT $2 OFFSET $3
    `;
    const r = await pool.query(q, [params.societyId, params.limit, params.offset]);
    return r.rows;
  }
}
