...
class AdminRepository {
    ...
    `;
        const r = await db_1.pool.query(q, [societyId]);
        return r.rows[0];
    }
    static async listUsers(params) {
      ...
      LIMIT $4 OFFSET $5
    `;
        const r = await db_1.pool.query(q, [params.societyId, params.role ?? null, params.status ?? null, params.limit, params.offset]);
        return r.rows;
    }
    ...
    static async updateUser(params) {
        ...
        const sets = [];
        const vals = [params.userId, params.societyId];
        let idx = 3;
    ...
    static async listBlocks(societyId) {
        const r = await db_1.pool.query(`SELECT * FROM blocks WHERE society_id=$1 ORDER BY name`, [societyId]);
        return r.rows;
    }
    static async createBlock(societyId, name) {
        const r = await db_1.pool.query(`INSERT INTO blocks (society_id, name) VALUES ($1,$2) RETURNING *`, [societyId, name]);
        return r.rows[0];
    }
    ...
    `;
        const r = await db_1.pool.query(q, [params.societyId, params.blockId ?? null]);
        return r.rows;
    }
    static async createFlat(params) {
        // ensure block belongs to society
        const b = await db_1.pool.query(`SELECT 1 FROM blocks WHERE id=$1 AND society_id=$2`, [params.blockId, params.societyId]);
        if (!b.rowCount)
            return null;
        const r = await db_1.pool.query(`INSERT INTO flats (block_id, flat_number, floor) VALUES ($1,$2,$3) RETURNING *`, [params.blockId, params.flatNumber, params.floor ?? null]);
        return r.rows[0];
    }
}
